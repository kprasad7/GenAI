...
MISTRAL_API_KEY = os.getenv("MISTRAL_API_KEY")  # Get API key from environment
...
def extract_code_from_response(response: str) -> str:
    ...
    # Use triple-quoted raw string for multi-line regex
    pattern = r'''```[a-zA-Z]*
(.*?)
```'''
...
class MessageBus:
    ...
    def send(self, message: AgentMessage):
        ...
        print(f"📨 [{message.sender} -> {message.recipient}] {message.message_type}: {message.content}")

    def receive(self, agent_name: str) -> List[AgentMessage]:
        received = [msg for msg in self.messages if msg.recipient == agent_name]
        # Clear received messages (simple model)
        self.messages = [msg for msg in self.messages if msg.recipient != agent_name]
        return received

# Initialize a global message bus
GLOBAL_MESSAGE_BUS = MessageBus()

class SpecializedAgent(AgentExecutor):
    """Base class for specialized agents that can send/receive messages."""
    def __init__(self, name: str, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.name = name

    def send_message(self, recipient: str, content: Any, msg_type: str = "task"):
        msg = AgentMessage(sender=self.name, recipient=recipient, content=content, message_type=msg_type)
        GLOBAL_MESSAGE_BUS.send(msg)

    def check_inbox(self) -> List[AgentMessage]:
        return GLOBAL_MESSAGE_BUS.receive(self.name)
...
class DockerPlatformTool(BaseTool):
        ...
        try:
            if action == "create_dockerfile":
                ...
                if result.returncode == 0:
                    ...
                    return f"❌ Failed to build Docker image: {stderr.decode()}"
            elif action == "run":
                result = await asyncio.create_subprocess_exec(
                    "docker", "run", "-d", "-p", "8000:8000", "--name", f"{app_name}_container", app_name,
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE
                )
                stdout, stderr = await result.communicate()
                if result.returncode == 0:
                    ...
                    return f"❌ Failed to run Docker container: {stderr.decode()}"
                ...
                if result.returncode == 0:
                    ...
                    return f"❌ Failed to start services: {stderr.decode()}"
            elif action == "stop":
                result = await asyncio.create_subprocess_exec(
                    "docker", "stop", f"{app_name}_container",
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE
                )
                stdout, stderr = await result.communicate()
                if result.returncode == 0:
                    ...
                    return f"❌ Failed to stop container: {stderr.decode()}"
                ...
                stdout, stderr = await result.communicate()
                if result.returncode == 0:
                    return f"✅ Docker is available: {stdout.decode().strip()}"
                else:
    ...
    def _run(self, action: str, app_name: str = "app", language: str = "python") -> str:
        ...
        return self._arun(action, app_name, language)
...
''')
    ...
    def _run(self, action: str, packages: str = "", requirements_file: str = "requirements.txt") -> str:
        return self._arun(action, packages, requirements_file)
...
class PlatformAgentTool(BaseTool):
    ...
    async def _arun(self, action: str, error_message: str = "", filename: str = "", app_name: str = "app") -> str:
        try:
            if action == "setup_docker_environment":
                ...
                docker_check = await docker_tool._arun("check_docker")
                results.append(docker_check)
                if "not available" in docker_check:
                    return "❌ Docker is not available. Please install Docker first."
                dockerfile_result = await docker_tool._arun("create_dockerfile", app_name, "python")
                results.append(dockerfile_result)
                compose_result = await docker_tool._arun("create_compose", app_name, "python")
                ...
                dep_result = await dep_tool._arun("auto_detect")
                return f"Requirements Creation Result:\n{dep_result}"

            elif action == "build_and_run":
                return await self._build_and_run_with_retry(app_name)
            elif action == "deploy_until_success":
                return await self._deploy_until_success(app_name)
                ...
                if missing_modules:
                    ...
                    dep_tool = DependencyManagerTool()
                    result = await dep_tool._arun("add_packages", ",".join(packages_to_add))
                    return f"✅ Resolved import errors by adding packages to requirements.txt:/n{', '.join(packages_to_add)}{result}"
                else:
                ...
                if "command not found" in error_message.lower():
                    ...
                    if container_match:
                        ...
                        if result.returncode == 0:
                            ...
                            return f"❌ Failed to remove container '{container_name}': {stderr.decode()}"
    ...
    def _run(self, action: str, error_message: str = "", filename: str = "", app_name: str = "app") -> str:
        return self._arun(action, error_message, filename, app_name)

    async def _build_and_run_with_retry(self, app_name: str, max_retries: int = 3) -> str:
        docker_tool = DockerPlatformTool()
        results = []
        for attempt in range(1, max_retries + 1):
            print(f"🔄 Deployment attempt {attempt}/{max_retries}")
            build_result = await docker_tool._arun("build", app_name)
            results.append(f"Attempt {attempt} - Build: {build_result}")
            if "successfully" in build_result:
                run_result = await docker_tool._arun("run", app_name)
                results.append(f"Attempt {attempt} - Run: {run_result}")
                if "started" in run_result:
                    return f"✅ SUCCESS! Application deployed successfully on attempt {attempt}/n" + "\n".join(results)

                elif "already in use" in run_result:
                    cleanup_result = await self._handle_container_conflict(run_result, app_name)
                    results.append(f"Attempt {attempt} - Cleanup: {cleanup_result}")
                    if "✅" in cleanup_result:
                        retry_run_result = await docker_tool._arun("run", app_name)
                        results.append(f"Attempt {attempt} - Retry Run: {retry_run_result}")
                        if "started" in retry_run_result:
                            return f"✅ SUCCESS! Application deployed successfully after cleanup on attempt {attempt}/n" + "\n".join(results)

            if attempt < max_retries:
                print(f"⏳ Waiting before retry...")
                await asyncio.sleep(2)
        return f"❌ FAILED! Could not deploy application after {max_retries} attempts/n" + "\n".join(results)


    async def _deploy_until_success(self, app_name: str, max_attempts: int = 10) -> str:
        docker_tool = DockerPlatformTool()
        dep_tool = DependencyManagerTool()
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                deployment_result = await self._build_and_run_with_retry(f"{app_name}_v{attempt}")
                if "SUCCESS" in deployment_result:
                    return f"🎉 Application successfully deployed on attempt {attempt}!{deployment_result}"
            except Exception as e:
    ...
    async def _handle_container_conflict(self, error_message: str, app_name: str) -> str:
        import re
        container_match = re.search(r'container name "([^"]+)"', error_message)
        if container_match:
            container_name = container_match.group(1)
...
class GenerateCodeTool(BaseTool):
    ...
    async def _arun(self, idea: str, frontend_framework: str = "auto") -> str:
        return self._run(idea, frontend_framework)
...
class FrontendBuildTool(BaseTool):
    ...
    async def _arun(self, action: str, framework: str = "html", build_dir: str = "dist") -> str:
        return self._run(action, framework, build_dir)
...
class IntegrationHandlerTool(BaseTool):
    ...
    async def _arun(self, integration_type: str, action: str = "check") -> str:
        return self._run(integration_type, action)
...
class CLIMonitorTool(BaseTool):
    ...
    async def _arun(self, action: str, command: str = "", output: str = "", error: str = "") -> str:
        return self._run(action, command, output, error)
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if "error" in logs.lower() or "failed" in logs.lower():
                    return f"⚠️ Issues detected in logs:{logs}{self._analyze_cli_error(logs, '')}"
                else:
                    return f"✅ Container running normally:{logs}"
            else:
...
class IntelligentFixTool(BaseTool):
    ...
    async def _arun(self, action: str, filename: str = "", error_type: str = "", error_message: str = "") -> str:
        return self._run(action, filename, error_type, error_message)
    ...
    def _fix_code_error(self, filename: str, error_message: str) -> str:
        ...
        if "syntaxerror" in error_message.lower():
            if "expected ':'" in error_message:
                content = re.sub(r'(if|elif|else|for|while|def|class|try|except|finally|with)\s+([^:]+)(?<!:)\s*', r'\1 \2:', content)
...
class AgentChainWorkflowTool(BaseTool):
    ...
    async def _arun(self, action: str, project_name: str = "", user_request: str = "") -> str:
        return self._run(action, project_name, user_request)
    ...
    def _intelligent_planning_phase(self, user_request: str) -> str:
        system_msg = """You are a senior software architect. Analyze the user's request and generate a detailed, structured development plan.
Output a JSON object with the following structure:
{
    "frontend_framework": "react|vue|angular|html",
    "backend_technology": "fastapi|flask",
    "key_features": ["feature1", "feature2"],
    "required_integrations": [
        {"name": "payment", "type": "dummy", "notes": "Use Stripe mock"},
        {"name": "database", "type": "local", "notes": "Use SQLite"}
    ],
    "file_structure": ["app.py", "requirements.txt", "src/App.js", ...],
    "deployment_strategy": "docker-compose with auto-restart"
}
Be specific and comprehensive.
"""
        messages = [SystemMessage(content=system_msg), HumanMessage(content=f"Create a development plan for: {user_request}")]
        response = llm.invoke(messages)
        ...
        try:
            ...
            if json_match:
                ...
                plan_json_str = raw_content
            plan = json.loads(plan_json_str)
            output_lines = ["🧠 Intelligent Plan Generated:"]
            output_lines.append(f"  Frontend: {plan.get('frontend_framework', 'N/A')}")
            output_lines.append(f"  Backend: {plan.get('backend_technology', 'N/A')}")
            output_lines.append(f"  Key Features: {', '.join(plan.get('key_features', []))}")
            output_lines.append("  Integrations:")
            for integration in plan.get('required_integrations', []):
                output_lines.append(f"    - {integration['name']} ({integration['type']}): {integration.get('notes', '')}")
            output_lines.append(f"  Deployment: {plan.get('deployment_strategy', 'N/A')}")
            return "\n".join(output_lines)
        except (json.JSONDecodeError, KeyError) as e:
            return f"⚠️  Failed to generate structured plan. Using basic analysis. Error: {e}"

    def _code_generation_phase(self, project_name: str, user_request: str) -> str:
        generation_results = []
        code_generator = GenerateCodeTool()
        generation_result = code_generator._run(user_request, "react")
    ...
    def _final_verification_phase(self, project_name: str) -> str:
        ...
        if accessible_ports and "✅ Container is running" in "".join(verification_results):
            verification_results.append("🎉 DEPLOYMENT SUCCESSFUL! Application is ready for use.")
        else:
            verification_results.append("⚠️ Deployment partially successful - manual verification recommended")
    ...
    def _continuous_monitor_and_fix(self, project_name: str) -> str:
        ...
        for cycle in range(max_monitoring_cycles):
            ...
            if cycle < max_monitoring_cycles - 1:
                time.sleep(5)
        monitor_results.append("🏁 Continuous monitoring cycle complete")
...
class FixCodeTool(BaseTool):
    ...
    async def _arun(self, filename: str, error: str) -> str:
        return self._run(filename, error)
    ...
    def _run(self, filename: str, error: str) -> str:
        ...
        context_str = f"File: {filename}Error: {error}Code context:"
...
def save_agent_state(memory: ConversationBufferMemory, filename: str = "agent_state.json"):
    """Save the agent's conversation memory to a file."""
    try:
        state = {
            "chat_history": [
                {
...
def load_agent_state(memory: ConversationBufferMemory, filename: str = "agent_state.json"):
    """Load the agent's conversation memory from a file."""
    try:
        if not os.path.exists(filename):
            print(f"ℹ️  No saved state found at {filename}")
            return
        with open(filename, 'r', encoding='utf-8') as f:
            state = json.load(f)
...
- Agent chain workflow orchestration for complex scenarios
...
START COMMAND: Always begin with agent_chain_workflow action "full_copilot_chain" to initiate the intelligent Copilot-like development workflow."""),
    ...
    try:
        ...
        print(f"🎉 SUCCESS! Agent completed its mission. Final output: {response['output']}")

        save_agent_state(memory)  # Save state after completion
    except Exception as e:
        print(f"❌ Error: {e}")
        load_agent_state(memory)  # Load state on error for recovery
